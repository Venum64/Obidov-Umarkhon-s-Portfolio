            ...
            <header className={`${h.header} ${isScrolled ? h.scrolled : ''}`}>
                <nav className={`container ${h.header__nav}`}>
                    <a href="#" className={h.header__logo}>
                        <img src={isDark ? logoWh : logoBl} alt="logo" />
                    </a>
                    <ul className={h.header__nav_list}>
                        <li><a href="#about" className={h.header__nav_list_item_link}>{t('navigation.about')}</a></li>
                        <li><a href="#skills" className={h.header__nav_list_item_link}>{t('navigation.skills')}</a></li>
                        <li><a href="#services" className={h.header__nav_list_item_link}>{t('navigation.services')}</a></li>
                        <li><a href="#projects" className={h.header__nav_list_item_link}>{t('navigation.projects')}</a></li>
                        <li><a href="#contacts" className={h.header__nav_list_item_link}>{t('navigation.contacts')}</a></li>
                    </ul>
                    <div className={h.header__nav_icons}>
                        <button className={h.header__nav_btn} onClick={changeLanguage}>{language}</button>
                        <span className={h.header__nav_icon} onClick={Theme}>
                            <i className={isDark ? "fa-sharp fa-regular fa-moon" : "fa-sharp fa-solid fa-moon"}
                               style={{ color: isDark ? "#fff" : "#282828" }}></i>
                        </span>
                    </div>
                </nav>
            </header>
            ...
            <section className={h.review}>
                <div className={`container ${h.review__content}`}>
                    <div className={h.review__l}>
                        <h2 className={`${h.review__l_h} from-left speed`} data-speed='1100'>{t('hero.myname')}</h2>
                        ...
                        <div className={`${h.review__l_btns} speed from-left`} data-speed='1250'>
                            <a href="#contacts" className={h.review__l_btn_a}><button className={h.review__l_btn1}>{t('navigation.contacts')}</button></a>
                            <a href="#projects" className={h.review__l_btn_a}><button className={h.review__l_btn2}>{t('hero.myprojects')}</button></a>
                        </div>
                        <div className={`${h.review__l_icons} speed from-left`} data-speed='1300'>
                            {iconsAdd.map((i) => (
                                <a href={i.l} className={h.review__l_a} key={i.id} target="_blank" rel="noreferrer">
                                    <span className={h.review__l_icon}><i className={i.i}></i></span>
                                </a>
                            ))}
                        </div>
                    </div>
